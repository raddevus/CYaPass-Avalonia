...
//        vm.allSiteKeys.Add(new SiteKey{Key="test3"});
      var vm = (MainWindowViewModel)DataContext;
      var currentSiteKey = vm.allSiteKeys.GetItemByKey(SiteKeys.SelectedItem.ToString());
      Console.WriteLine($"get type: {currentSiteKey?.GetType()}");
      var msg = new SiteKeyMsgBox("Make changes to the SiteKey & click the OK button.", currentSiteKey);
       bool dialogResult =  await msg.ShowDialog<bool>(this);
